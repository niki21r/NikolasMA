...
public class VertexCharacteristicsListTest extends AbstractSelectorConstraintTest {
	...
	protected Stream<SelectorTestCase> cases() {
		return Stream.concat(vertexCharacteristicsListDestinationCases(),
				vertexCharacteristicsListSourceCasesIfNonRecursive());
	}
	...
	/**
	 * Contains Test cases Vertex Characteristics List should not act recursive as a
	 * source selector
	 * 
	 * @return test cases
	 */
	private static Stream<SelectorTestCase> vertexCharacteristicsListSourceCasesIfNonRecursive() {
		return Stream.of(
				...
				new SelectorTestCase("Sink needs to remove label 2",
						...
						List.of(TRUE), List.of("(not " + nodeLabel(SINK, LABEL2) + ")")),
				new SelectorTestCase("Both need to remove their label",
						new ConstraintDSL().fromNode().withCharacteristic(LABELTYPE, List.of(LABEL1, LABEL2))
								.neverFlows().toVertex().create(),
						List.of("(not " + nodeLabel(SOURCE, LABEL1) + ")"),
						List.of("(not " + nodeLabel(SINK, LABEL2) + ")")),
				new SelectorTestCase("Empty list always satisfied for normal selector",
						...
						List.of(TRUE), List.of(TRUE)),
				new SelectorTestCase("Both need to add the label",
						new ConstraintDSL().fromNode().withoutCharacteristic(LABELTYPE, List.of(LABEL3)).neverFlows()
								.toVertex().create(),
						List.of(nodeLabel(SOURCE, LABEL3)), List.of(nodeLabel(SINK, LABEL3))),
				new SelectorTestCase(
	...
	/**
	 * Contains Test cases Vertex Characteristics List should act recursive as a
	 * source selector. Not used.
	 * 
	 * @return test cases
	 */
