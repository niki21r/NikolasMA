...
public final class ConstraintMapProvider {
	...
	private static AnalysisConstraint entryViaGatewayOnly() {
		return new ConstraintDSL().ofData().withLabel("Stereotype", "entrypoint").withoutLabel("Stereotype", "gateway")
				.neverFlows().toVertex().withCharacteristic("Stereotype", "internal").create();
	}

	private static AnalysisConstraint nonInternalGateway() {
		return new ConstraintDSL().ofData().neverFlows().toVertex().withCharacteristic("Stereotype", "gateway")
				.withCharacteristic("Stereotype", "internal").create();
	}

	private static AnalysisConstraint authenticatedRequest() {
		return new ConstraintDSL().ofData().withoutLabel("Stereotype", "authenticated_request").neverFlows().toVertex()
				.withCharacteristic("Stereotype", "internal").create();
	}

	private static AnalysisConstraint transformedEntry() {
		return new ConstraintDSL().ofData().withLabel("Stereotype", "entrypoint")
	...
	private static AnalysisConstraint encryptedInternals() {
		return new ConstraintDSL().ofData().withLabel("Stereotype", "internal")
				.withoutLabel("Stereotype", "encrypted_connection").neverFlows().toVertex().create();
	}

	private static AnalysisConstraint localLogging() {
		return new ConstraintDSL().ofData().neverFlows().toVertex().withCharacteristic("Stereotype", "internal")
				.withoutCharacteristic("Stereotype", "local_logging").create();
	}

	private static AnalysisConstraint logSanitization() {
		return new ConstraintDSL().ofData().neverFlows().toVertex().withCharacteristic("Stereotype", "local_logging")
				.withoutCharacteristic("Stereotype", "log_sanitization").create();
	}

	private static final Map<Integer, List<AnalysisConstraint>> CONSTRAINTS = Map.of(1,
			List.of(entryViaGatewayOnly(), nonInternalGateway()), 2, List.of(authenticatedRequest()), 4,
			List.of(transformedEntry()), 5, List.of(tokenValidation()), 7,
			List.of(encryptedEntry(), entryViaGatewayOnly(), nonInternalGateway()), 8, List.of(encryptedInternals()),
			10, List.of(localLogging()), 11, List.of(localLogging(), logSanitization()));
	...
	public static List<Integer> variantIds() {
		return CONSTRAINTS.keySet().stream().sorted(Comparator.naturalOrder()).toList();
	}
