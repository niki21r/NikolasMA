...
class ContainsSelectorTest extends UtilTestBase {
	...
	static Stream<Arguments> vertexNameCases() {
		return Stream.of(
				...
				Arguments.of(new ConstraintDSL().fromNode().neverFlows().toVertex().create(), false),
				// destination vertex selector contains VertexNameSelector via
				// toVertex().withName(...)
				Arguments.of(new ConstraintDSL().fromNode().neverFlows().toVertex().withVertexName("v1").create(),
						true));
	}
